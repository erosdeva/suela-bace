...
export default function BookDetailsModal({ book, onClose }: BookDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neutral-900/40 backdrop-blur-md">
      ...
      <motion.div
        ...
        className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white border border-neutral-200 rounded-xl shadow-2xl text-neutral-800 flex flex-col md:flex-row z-10"
        id={`book-modal-card-${book.id}`}
      >
        {/* Left Side: Sticky Cover & Buy CTA */}
        <div className="w-full md:w-2/5 bg-neutral-50 p-6 flex flex-col items-center justify-between border-b md:border-b-0 md:border-r border-neutral-200">
          <div className="w-full flex justify-center py-4">
            <div className="relative group shadow-xl border-4 border-white rounded-lg overflow-hidden max-w-[200px] md:max-w-full">
              <img
                src={book.coverImage}
                alt={book.title}
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-neutral-900/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center p-4">
                <span className="text-xs tracking-widest uppercase font-mono text-red-100 flex items-center gap-1">
          ...
          <div className="w-full space-y-4 mt-6">
            <div className="text-center md:text-left">
              ...
              <div className="mt-2 space-y-2 text-xs text-neutral-600 font-mono">
                {book.publishDate && (
        ...
        <div className="w-full md:w-3/5 p-6 md:p-8 flex flex-col justify-between">
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-2 rounded-full hover:bg-neutral-100 transition-colors text-neutral-500 hover:text-neutral-900"
            aria-label="Close"
          ...
          <div className="space-y-6">
            <div>
              <p className="text-xs uppercase tracking-widest text-red-600 font-mono">Literary Masterpiece</p>
              <h3 className="text-2xl md:text-3xl font-serif font-bold text-neutral-900 tracking-tight mt-1">
                {book.title}
              </h3>
              <p className="italic text-neutral-600 mt-2 font-serif text-sm">
                "{book.subtitle}"
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {book.themes.map((theme) => (
                <span
                  key={theme}
                  className="px-2.5 py-1 text-[10px] font-mono uppercase bg-neutral-100 text-neutral-700 border border-neutral-200 rounded-full"
                >
                  {theme}
                </span>
              ))}
            ...
            <div className="prose prose-sm max-w-none text-neutral-700 space-y-4">
              ...
              <p className="leading-relaxed whitespace-pre-line text-sm md:text-base font-serif font-light text-neutral-700">
          ...
          <div className="mt-8 pt-4 border-t border-neutral-200 text-right">
            <button
              onClick={onClose}
              className="text-xs tracking-widest uppercase font-mono text-neutral-500 hover:text-neutral-900 transition-colors"
